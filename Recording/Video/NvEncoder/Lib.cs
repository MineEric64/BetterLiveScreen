...
namespace BetterLiveScreen.Recording.Video.NvEncoder
{
    ...
    {
        public const string dllName = "uNvEncoder.dll";
        ...
        [DllImport(dllName, EntryPoint = "uNvEncoderGetSharpDevice")]
        public static extern IntPtr GetSharpDevice();
        [DllImport(dllName, EntryPoint = "uNvEncoderSetSharpDevice")]
        public static extern void SetSharpDevice(IntPtr device);
        [DllImport(dllName, EntryPoint = "uNvEncoderGetDevice")]
        public static extern IntPtr GetDevice();

        public static int Create(EncoderDesc desc, Device device)
        {
            SetSharpDevice(device.NativePointer);

            int size = Marshal.SizeOf(typeof(EncoderDesc));
